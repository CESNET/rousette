...
#define EXPECT(DATA, SEQ) expectations.emplace_back(NAMED_REQUIRE_CALL(netconfWatcher, data(DATA)).IN_SEQUENCE(SEQ));
...
TEST_CASE("Subscribed notifications")
{
  ...
  "ietf-subscribed-notifications:output": {
    ...
    EXPECT_NOTIFICATION(R"({"ietf-yang-push:push-change-update":{"id":1,"datastore-changes":{"yang-patch":{"patch-id":"patch-1","edit":[{"edit-id":"edit-1","operation":"create","target":"/example:top-level-leaf","value":{"example:top-level-leaf":"11"}}]}}}})", seq1);
    EXPECT_NOTIFICATION(R"({"ietf-yang-push:push-change-update":{"id":1,"datastore-changes":{"yang-patch":{"patch-id":"patch-2","edit":[{"edit-id":"edit-1","operation":"replace","target":"/example:top-level-leaf","value":{"example:top-level-leaf":"42"}}]}}}})", seq1);
    ...
    std::jthread notificationThread = std::jthread(wrap_exceptions_and_asio(bg, io, [&]() {
        ...
        notifSession.setItem("/example:top-level-leaf", "11");
