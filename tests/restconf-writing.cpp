    ...
    "error": [
      {
        ...
        "error-message": "Validation failure: Can't parse data: LY_EVALID"
            ...
            REQUIRE(put("/example:top-level-leaf", R"({"example:a": {}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{400, jsonHeaders, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "protocol",
        ...
        SECTION("Create and modify a leaf")
        {
            ...
            SECTION("Leaf in a container")
            {
                ...
                REQUIRE(put("/example:two-leafs/a", R"({"example:a": "a-value"}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{201, jsonHeaders, ""});

                EXPECT_CHANGE(MODIFIED("/example:two-leafs/a", "another-a-value"));
                REQUIRE(put("/example:two-leafs/a", R"({"example:a": "another-a-value"}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
            }

            SECTION("Repeated insertion") {
                EXPECT_CHANGE(CREATED("/example:top-level-leaf", "str"));
                REQUIRE(put("/example:top-level-leaf", R"({"example:top-level-leaf": "str"}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{201, jsonHeaders, ""});
                REQUIRE(put("/example:top-level-leaf", R"({"example:top-level-leaf": "str"}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
            }
        }
        ...
        SECTION("Container operations")
        {
            ...
            REQUIRE(put("/example:two-leafs", R"({"example:two-leafs": {"a": "a-val", "b": "b-val"}})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
            ...
            SECTION("Overwrite container with only one child, the second gets deleted")
            {
                ...
                REQUIRE(put("/example:two-leafs", R"({"example:two-leafs": {"b": "new-b-val"}})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
            }

            SECTION("Modify one leaf")
            {
                EXPECT_CHANGE(MODIFIED("/example:two-leafs/b", "new-b-val"));
                REQUIRE(put("/example:two-leafs/b", R"({"example:b": "new-b-val"})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
            }
            ...
            SECTION("Set container to empty container (delete)")
            {
                ...
                REQUIRE(put("/example:two-leafs", R"({"example:two-leafs": {}})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
            }
        }
    ...
    "error": [
      {
        ...
        SECTION("List operations")
        {
            ...
            SECTION("Insert a larger portion of data")
            {
                ...
                REQUIRE(put("/example:tlc/list=large", R"({"example:list":[{"name": "large", "choice2": "large", "example:nested": [{"first": "1", "second": 2, "third": "3"}]}]})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{201, jsonHeaders, ""});
            }
            ...
            SECTION("Insert into the list having multiple keys")
            {
                ...
                REQUIRE(put("/example:tlc/list=libyang/nested=11,12,13", R"({"example:nested": [{"first": "11", "second": 12, "third": "13"}]}]})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{201, jsonHeaders, ""});
            }

            SECTION("Modify a leaf in a list entry")
            {
                EXPECT_CHANGE(MODIFIED("/example:tlc/list[name='libyang']/choice1", "restconf"));
                REQUIRE(put("/example:tlc/list=libyang/choice1", R"({"example:choice1": "restconf"})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
            }
            ...
            SECTION("Overwrite a list entry")
            {
                ...
                REQUIRE(put("/example:tlc/list=libyang/collection=4", R"({"example:collection": [4]})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{201, jsonHeaders, ""});
                ...
                REQUIRE(put("/example:tlc/list=libyang", R"({"example:list":[{"name": "libyang", "choice1": "idk", "collection": [1,2,3]}]})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
            }
    ...
    "error": [
      {
        ...
        "error-message": "Invalid data for PUT (list key mismatch between URI path and data)."
                ...
                REQUIRE(put("/example:tlc/list=netconf", R"({"example:list":[{"name": "netconf", "choice1": "nope"}, {"name": "sysrepo", "choice1": "bla"}]})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{400, jsonHeaders, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "protocol",
        ...
        "error-message": "Invalid data for PUT (list key mismatch between URI path and data)."
                ...
                REQUIRE(put("/example:tlc/list=libyang/collection=5", R"({"example:collection": [5, 42]})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{400, jsonHeaders, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "protocol",
