    ...
    "error": [
      {
        ...
        SECTION("List operations")
        {
            ...
            SECTION("Insert YANG identities")
            {
                ...
                REQUIRE(put(RESTCONF_DATA_ROOT "/example:list-with-identity-key=example:derived-identity,name", {AUTH_ROOT, CONTENT_TYPE_JSON}, R"({"example:list-with-identity-key": [{"name": "name", "type": "derived-identity", "text": "hehe"}]}]})") == Response{204, noContentTypeHeaders, ""});
    ...
    "error": [
      {
        ...
        "error-message": "Validation failure: Can't parse data: LY_EVALID"
                ...
                REQUIRE(put(RESTCONF_DATA_ROOT "/example:list-with-identity-key=example-types:another-derived-identity,name", {AUTH_ROOT, CONTENT_TYPE_JSON}, R"({"example:list-with-identity-key": [{"name": "name", "type": "example-types:another-derived-identity", "text": "blabla"}]}]})") == Response{201, noContentTypeHeaders, ""});

                // missing namespace in the data
                REQUIRE(put(RESTCONF_DATA_ROOT "/example:list-with-identity-key=example-types:another-derived-identity,name", {AUTH_ROOT, CONTENT_TYPE_JSON}, R"({"example:list-with-identity-key": [{"name": "name", "type": "another-derived-identity", "text": "blabla"}]}]})") == Response{400, jsonHeaders, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "protocol",
