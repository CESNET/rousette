    ...
    SECTION("PUT with NMDA")
    {
        ...
        SECTION("Read-only datastores")
        {
            ...
            REQUIRE(put(uri + "/example:top-level-leaf", R"({"example:top-level-leaf": "str"})", {CONTENT_TYPE_JSON, AUTH_ROOT}) ==
                    Response{405, {ACCESS_CONTROL_ALLOW_ORIGIN, CONTENT_TYPE_JSON, {"allow", "DELETE, GET, HEAD, OPTIONS, PATCH, POST, PUT"}, ACCEPT_PATCH}, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "application",
    ...
    "error": [
      {
        ...
        SECTION("Read-only datastores")
        {
            ...
            REQUIRE(post(uri + "/", R"({"example:top-level-leaf": "str"})", {CONTENT_TYPE_JSON, AUTH_ROOT}) ==
                    Response{405, {ACCESS_CONTROL_ALLOW_ORIGIN, CONTENT_TYPE_JSON, {"allow", "GET, HEAD, OPTIONS, PATCH, POST, PUT"}, ACCEPT_PATCH}, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "application",
