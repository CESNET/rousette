...
TEST_CASE("invoking actions and rpcs")
{
    ...
    "error": [
      {
        ...
        {
            REQUIRE(post(RESTCONF_OPER_ROOT "/example:test-rpc", R"({"example:input": {}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{500, jsonHeaders, R"eof({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "application",
        "error-tag": "operation-failed",
        "error-message": "Internal server error due to sysrepo exception: Couldn't send RPC: SR_ERR_VALIDATION_FAILED\u000A Mandatory node \"i\" instance does not exist. (Data location \"/example:test-rpc\".) (SR_ERR_LY)\u000A RPC input validation failed. (SR_ERR_VALIDATION_FAILED)"
      }
    ]
  }
}
        ...
        {
            REQUIRE(post(RESTCONF_OPER_ROOT "/example:test-rpc", "", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{500, jsonHeaders, R"eof({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "application",
  ...
  "example:output": {
    "o": "some-output-string"
        ...
        {
            REQUIRE(post(RESTCONF_DATA_ROOT "/example:tlc/list=666/example-action", R"({"example:input": {"example:i": "ahoj"}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{500, jsonHeaders, R"eof({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "application",
        "error-tag": "operation-failed",
        "error-message": "Internal server error due to sysrepo exception: Couldn't send RPC: SR_ERR_VALIDATION_FAILED\u000A Nested operation \"example-action\" data parent does not exist in the operational datastore. (SR_ERR_VALIDATION_FAILED)"
      }
    ]
  }
}
