...
TEST_CASE("invoking actions and rpcs")
{
  ...
  "example:output" : {
    ...
    "out2": "some-output-string-2"
            ...
            REQUIRE(post(RESTCONF_OPER_ROOT "/example:test-rpc-no-output", R"({"example:input": {"number": 42, "string":"ahoj"}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});

            REQUIRE_CALL(rpcCall, rpcCall("/example:test-rpc-no-input-no-output", std::map<std::string, std::string>({})));
            REQUIRE(post(RESTCONF_OPER_ROOT "/example:test-rpc-no-input-no-output", "", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
        }
    ...
    "error": [
      {
        ...
        {
            REQUIRE(post(RESTCONF_OPER_ROOT "/example:test-rpc", R"({"example:input": {}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{400, jsonHeaders, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "protocol",
        ...
        {
            REQUIRE(post(RESTCONF_OPER_ROOT "/example:test-rpc", "", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{400, jsonHeaders, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "protocol",
  ...
  "example:output" : {
    "o": "some-output-string"
        ...
        {
            REQUIRE(post(RESTCONF_DATA_ROOT "/example:tlc/list=666/example-action", R"({"example:input": {"example:i": "ahoj"}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{200, jsonHeaders, R"({
  "example:output" : {
    "o": "some-output-string"
  }
})"});
