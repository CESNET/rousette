...
TEST_CASE("invoking actions and rpcs")
{
    ...
    auto rpc3 = srSess.onRPCAction("/example:test-rpc-no-input-no-output", [&](sysrepo::Session, auto, auto path, auto, auto, auto, auto) {
        rpcCall.rpcCall(path, {});
        return sysrepo::ErrorCode::Ok;
    });

    auto rpc4 = srSess.onRPCAction("/example:tlc/list/example-action", [&](sysrepo::Session, auto, auto path, libyang::DataNode input, auto, auto, libyang::DataNode output) {
        rpcCall.rpcCall(path, nodesToMap(input));
        output.newPath("o", "some-output-string", libyang::CreationOptions::Output);
        return sysrepo::ErrorCode::Ok;
    });
    ...
    SECTION("RPC")
    {
        SECTION("Basic calls")
        {
            ...
            CHECK(post(RESTCONF_OPER_ROOT "/example:test-rpc", R"({"example:input": {"i":"ahoj"}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{200, jsonHeaders, R"({
  "example:output" : {
    "o": "some-output-string"
            ...
            CHECK(post(RESTCONF_OPER_ROOT "/example:test-rpc-no-output", R"({"example:input": {"number": 42, "string":"ahoj"}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});

            REQUIRE_CALL(rpcCall, rpcCall("/example:test-rpc-no-input-no-output", std::map<std::string, std::string>({})));
            CHECK(post(RESTCONF_OPER_ROOT "/example:test-rpc-no-input-no-output", "", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
        }
    ...
    "error": [
      {
        ...
        "error-message": "'/example:test-rpc' is not a data resource"
      }
    ]
  }
}
    ...
    "error": [
      {
        ...
        SECTION("No input nodes when required")
        {
            CHECK(post(RESTCONF_OPER_ROOT "/example:test-rpc", R"({"example:input": {}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{400, jsonHeaders, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "protocol",
        ...
        SECTION("No input when required")
        {
            CHECK(post(RESTCONF_OPER_ROOT "/example:test-rpc", "", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{400, jsonHeaders, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "protocol",
  ...
  "ietf-restconf:errors": {
    ...
    SECTION("Actions")
    {
        SECTION("Basic calls")
        {
            ...
            CHECK(post(RESTCONF_DATA_ROOT "/example:tlc/list=1/example-action", R"({"example:input": {"example:i": "ahoj"}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{200, jsonHeaders, R"({
  "example:output" : {
    "o": "some-output-string"
  }
})"});
        ...
        {
            CHECK(post(RESTCONF_DATA_ROOT "/example:tlc/list=666/example-action", R"({"example:input": {"example:i": "ahoj"}}")", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{200, jsonHeaders, R"({
  "example:output" : {
    "o": "some-output-string"
  }
})"});
    ...
    "error": [
      {
        "error-type": "application",
        "error-tag": "operation-failed",
        "error-message": "'/example:tlc' is not an operations resource"
      }
    ]
  }
}
