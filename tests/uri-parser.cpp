...
TEST_CASE("URI path parser")
{
    ...
    SECTION("Streams")
    {
...

        {
            ...
            REQUIRE(std::holds_alternative<RestconfStreamRequest::SubscribedStream>(type));
            REQUIRE(std::get<RestconfStreamRequest::SubscribedStream>(type).uuid == boost::uuids::string_generator()("a40f0a50-061a-4832-a6ac-c4db7df81a10"));
            REQUIRE(queryParams.empty());
        }
