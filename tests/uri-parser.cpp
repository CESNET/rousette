...
std::ostream& operator<<(std::ostream& os, const rousette::restconf::ApiIdentifier& obj)
{
    os << "ApiIdentifier{";
    if (obj.prefix) {
        os << "prefix='" << *obj.prefix << "', ";
    }
    return os << "ident='" << obj.identifier << "'}";
}

std::ostream& operator<<(std::ostream& os, const rousette::restconf::PathSegment& obj)
{
    return os << "Segment{" << obj.apiIdent << " "
              << "keys=" << obj.keys << "}";
}

std::ostream& operator<<(std::ostream& os, const rousette::restconf::ResourcePath& obj)
{
    os << "[";
    std::copy(obj.getSegments().begin(), obj.getSegments().end(), std::experimental::make_ostream_joiner(os, ", "));
    return os << "]";
}
}
