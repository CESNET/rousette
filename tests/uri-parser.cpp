...
TEST_CASE("URI path parser")
{
    ...
    SECTION("Translation to libyang path")
    {
        ...
        SECTION("Contextually valid paths")
        {
            SECTION("GET and PUT")
            {
                for (const auto& [httpMethod, expectedRequestType] : {std::pair<std::string, RestconfRequest::Type>{"GET", RestconfRequest::Type::GetData}, {"PUT", RestconfRequest::Type::CreateOrUpdate}}) {
