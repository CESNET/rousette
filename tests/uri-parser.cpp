...
TEST_CASE("URI path parser")
{
    ...
    SECTION("Invalid URIs")
    {
        for (const auto& [uriPath, position, expectedToken] : {
                 std::tuple<std::string, unsigned, std::string>{"/restconf/foo", 10, "resource"},
                 {"/restconf/foo/foo:bar", 10, "resource"},
                 {"/restconf/data/foo", 18, "':'"},
                 {"/restconf/data/foo:", 19, "identifier"},
                 {"/restconf/data/:bar", 15, "eoi"},
                 {"/restconf/data/333:666", 15, "eoi"},
                 {"/restconf/data/foo:bar/lst==", 27, "eoi"},
                 {"/restconf/data/foo:bar/lst==key", 27, "eoi"},
                 {"/restconf/data/foo:bar/=key", 23, "identifier"},
                 {"/restconf/data/foo:bar/lst=key1,,,,=", 35, "eoi"},
                 {"/restconf/data/foo:bar/X=Y=instance-value", 26, "eoi"},
                 {"/restconf/data/foo:bar/:baz", 23, "identifier"},
                 {"/restconf/data/foo:list=A%xyZ", 26, "twoHexDigits"},
                 {"/restconf/data/foo:list=A%0zZ", 26, "twoHexDigits"},
                 {"/restconf/data/foo:list=A%%1Z", 26, "twoHexDigits"},
                 {"/restconf/data/foo:list=A%%25Z", 26, "twoHexDigits"},
                 {"/restconf/data/foo:list=A%2", 26, "twoHexDigits"},
                 {"/restconf/data/foo:list=A%2,", 26, "twoHexDigits"},
                 {"/restconf/data/foo:bar/list1=%%", 30, "twoHexDigits"},
                 {"/restconf/data/foo:bar/list1=module:smth", 35, "eoi"},
                 {"/restconf/data/foo:bar/", 23, "identifier"},
                 {"/restconf/data/ foo : bar", 15, "eoi"},
                 {"/rest conf/data / foo:bar", 1, "\"restconf\""},
                 {"/restconf/da ta/foo:bar", 10, "resource"},
                 {"/restconf/data / foo:bar = key1", 14, "eoi"},
                 {"/restconf/data / foo:bar =key1", 14, "eoi"},
                 {"/restconf/ data", 10, "resource"},
                 {"/restconf /data", 9, "resources"},
                 {"/restconf  data", 9, "resources"},
                 ...
                 {"/restconf/yang-library", 10, "resource"},
    ...
    SECTION("Translation to libyang path")
    {
        SECTION("Contextually valid paths")
        {
            ...
            {
                auto [action, datastore, path, queryParams] = rousette::restconf::asRestconfRequest(ctx, "POST", "/restconf/ds/ietf-datastores:operational/example:tlc/list=hello-world/example-action");
        ...
        SECTION("Contextually invalid paths")
        {
            ...
            SECTION("GET, PUT, DELETE, POST (data), PATCH")
            {
                ...
                SECTION("Unparseable URI")
                {
                    ...
                    expectedErrorMessage = "Syntax error in URI path at position 15: expected eoi";
    ...
    SECTION("query params")
    {
        ...
        SECTION("Parsing")
        {
            ...
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("a=b"), 0, "eoi");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("Depth=1"), 0, "eoi");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("depth=-1"), 6, "depthParam");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("depth=0"), 6, "depthParam");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("depth=65536"), 6, "depthParam");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("depth="), 6, "depthParam");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("depth=foo"), 6, "depthParam");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("="), 0, "eoi");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("&"), 0, "eoi");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("depth=1&"), 7, "eoi");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("a&b=a"), 0, "eoi");
            ...
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("with-defaults="), 14, "withDefaultsParam");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("with-defaults=report"), 14, "withDefaultsParam");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("with_defaults=ahoj"), 0, "eoi");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("with-defaults=report_all"), 14, "withDefaultsParam");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("with-defaults=depth=3"), 14, "withDefaultsParam");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("with-defaults=&depth=3"), 14, "withDefaultsParam");
            ...
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("content=ahoj"), 8, "contentParam");
            ...
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("insert=foo"), 7, "insertParam");
            ...
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("insert=uwu"), 7, "insertParam");
            ...
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("filter=/example:mod/statistics[errors>0]&"), 40, "eoi");
            QUERY_PARAMS_SYNTAX_ERROR(parseQueryParams("filter=/example:mod[name='&amp;']"), 26, "eoi");
        ...
        SECTION("Full requests with validation")
        {
            ...
            SECTION("filter")
            {
                ...
                REQUIRE(std::holds_alternative<NotificationStreamRequest>(resp));
                REQUIRE(std::get<NotificationStreamRequest>(resp).queryParams == QueryParams({{"filter", "/asd"s}}));
            ...
            SECTION("start-time")
            {
                ...
                REQUIRE(std::holds_alternative<NotificationStreamRequest>(resp));
                REQUIRE(std::get<NotificationStreamRequest>(resp).queryParams == QueryParams({{"start-time", "2024-01-01T01:01:01Z"s}}));
            ...
            SECTION("stop-time")
            {
                ...
                REQUIRE(std::holds_alternative<NotificationStreamRequest>(resp));
                REQUIRE(std::get<NotificationStreamRequest>(resp).queryParams == QueryParams({{"stop-time", "2024-01-01T01:01:01Z"s}}));
            ...
            REQUIRE_THROWS_WITH_AS(asRestconfRequest(ctx, "GET", "/restconf/data/example:tlc", "hello=world"),
                                   serializeErrorResponse(400, "protocol", "invalid-value", "Syntax error in URI querystring at position 0: expected eoi").c_str(),
                                   rousette::restconf::ErrorResponse);
        }
    }
    ...
    SECTION("Streams")
    {
...

        {
            ...
            REQUIRE(std::holds_alternative<NotificationStreamRequest>(req));
            REQUIRE(std::get<NotificationStreamRequest>(req).stream == "NETCONF");
            REQUIRE(std::get<NotificationStreamRequest>(req).encoding == libyang::DataFormat::XML);
            REQUIRE(std::get<NotificationStreamRequest>(req).queryParams.empty());
        }

        {
            ...
            REQUIRE(std::holds_alternative<NotificationStreamRequest>(req));
            REQUIRE(std::get<NotificationStreamRequest>(req).stream == "NETCONF");
            REQUIRE(std::get<NotificationStreamRequest>(req).encoding == libyang::DataFormat::JSON);
            REQUIRE(std::get<NotificationStreamRequest>(req).queryParams.empty());
        }

        {
            auto req = asRestconfStreamRequest("GET", "/streams/AHAhaha/JSON", "");
            REQUIRE(std::holds_alternative<NotificationStreamRequest>(req));
            REQUIRE(std::get<NotificationStreamRequest>(req).stream == "AHAhaha");
            REQUIRE(std::get<NotificationStreamRequest>(req).encoding == libyang::DataFormat::JSON);
            REQUIRE(std::get<NotificationStreamRequest>(req).queryParams.empty());
        }
        ...
        REQUIRE_THROWS_WITH_AS(asRestconfStreamRequest("GET", "/streams/NETCONF/xml", ""),
                               serializeErrorResponse(400, "protocol", "invalid-value", "Syntax error in URI path at position 17: expected streamFormat").c_str(),
                               rousette::restconf::ErrorResponse);
