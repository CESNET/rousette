...
TEST_CASE("URI path parser")
{
    ...
    SECTION("OPTIONS")
    {
        ...
        {
            std::string uri;
            std::optional<rousette::restconf::OptionsHeaders> expected;

            SECTION("Data resource")
            {
                expected = {"DELETE, GET, HEAD, OPTIONS, PATCH, POST, PUT", "application/yang-data+json;charset=utf-8,application/yang-data+xml;charset=utf-8"};
            ...
            {
                expected = {"OPTIONS, POST", std::nullopt};
                SECTION("RPC") { uri = "/restconf/operations/example:test-rpc"; }
                SECTION("Action") { uri = "/restconf/data/example:tlc/list=key/example-action"; }
            }
            SECTION("Datastore resource")
            {
                expected = {"GET, HEAD, OPTIONS, PATCH, POST, PUT", "application/yang-data+json;charset=utf-8,application/yang-data+xml;charset=utf-8"};
