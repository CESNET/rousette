...
TEST_CASE("SSE proxy for configured subscriptions")
{
    ...
    // Only the 'optics' group (i.e., the dwdm user) may consume the endpoint; everybody else is rejected by the
    // nacm:default-deny-all on the nacm-access-check node.
    ...
    SECTION("access is governed by NACM")
    {
