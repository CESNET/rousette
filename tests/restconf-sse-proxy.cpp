...
TEST_CASE("SSE proxy for configured subscriptions")
{
    ...
    {
        RestconfYangPushWatcher watcher1(srConn.sessionStart().getContext());
        RestconfYangPushWatcher watcher2(srConn.sessionStart().getContext());
        watcher1.setDataFormat(libyang::DataFormat::JSON);
        watcher2.setDataFormat(libyang::DataFormat::JSON);

        RestconfYangPushWatcher watcherDenied(srConn.sessionStart().getContext());
        ...
        SSEClient client1(io, SERVER_ADDRESS, SERVER_PORT, requestSent, watcher1, "/streams/rousette:sse-proxy/example", {AUTH_DWDM}, 6s);
        SSEClient client2(io, SERVER_ADDRESS, SERVER_PORT, requestSent, watcher2, "/streams/rousette:sse-proxy/example", {AUTH_ROOT}, 6s);
        SSEClient clientDenied(io, SERVER_ADDRESS, SERVER_PORT, requestSent, watcherDenied, "/streams/rousette:sse-proxy/example", {AUTH_NORULES}, 6s);
    ...
    SECTION("a new endpoint appears when it is configured")
    {
        ...
        RestconfYangPushWatcher watcher(srConn.sessionStart().getContext());
        ...
        SSEClient client(io, SERVER_ADDRESS, SERVER_PORT, requestSent, watcher, "/streams/rousette:sse-proxy/later", {AUTH_ROOT}, 6s);
        RUN_LOOP_WITH_EXCEPTIONS;
    }
    ...
    SECTION("a client is not disconnected by a configuration change elsewhere")
    {
        ...
        SSEClient client(io, SERVER_ADDRESS, SERVER_PORT, requestSent, watcher, "/streams/rousette:sse-proxy/example", {AUTH_ROOT}, 6s);
        RUN_LOOP_WITH_EXCEPTIONS;
    }
    ...
    SECTION("an endpoint removed from the configuration stops serving its clients")
    {
        ...
        SSEClient client(io, SERVER_ADDRESS, SERVER_PORT, requestSent, watcher, "/streams/rousette:sse-proxy/example", {AUTH_ROOT}, 2s);
        RUN_LOOP_WITH_EXCEPTIONS;
    }
