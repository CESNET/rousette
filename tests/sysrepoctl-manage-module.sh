...
while [[ $# -gt 0 ]]; do
  ...
  # parse and process all arguments up to next "YANG" directive into arguments for one sysrepotl call.
  CTL_ARGUMENTS=""
  if [[ "${MODE}" == "prepare" ]]; then
    ${SYSREPOCTL} --uninstall "${MODULE}" || true
    CTL_ARGUMENTS="--search-dirs ${YANG_DIR} --install ${YANG_FILE}"
  elif [[ "${MODE}" == "uninstall" ]]; then
    MODULE_LIST=( "${MODULE}" "${MODULE_LIST[@]}" ) # save for later; uninstall in reverse order
  fi

  while [[ $# -gt 0 && ${1} != "YANG" ]]; do
    # for uninstalling, we just need to skip these args
    ...
    if [[ "${1}" == "FEATURE" ]]; then
      ...
      if [[ "${MODE}" == "prepare" ]]; then
        CTL_ARGUMENTS="${CTL_ARGUMENTS} --enable-feature ${FEATURE}"
      fi
      ...
      if [[ "${MODE}" == "prepare" ]]; then
        ...
        CTL_ARGUMENTS="${CTL_ARGUMENTS} --init-data ${DATA_FILE}"
      fi
    fi
  done
  if [[ "${MODE}" == "prepare" ]]; then
