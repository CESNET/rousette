  ...
  "example:config-nonconfig": {
    ...
    SECTION("OPTIONS method")
    {
        ...
        REQUIRE(options(RESTCONF_DATA_ROOT "/example:tlc/list=a", {}) == Response{200,
                {ACCESS_CONTROL_ALLOW_ORIGIN, {"allow", "DELETE, GET, HEAD, OPTIONS, PATCH, POST, PUT"}, ACCEPT_PATCH}, ""});

        // ds root
        REQUIRE(options(RESTCONF_DATA_ROOT, {}) == Response{200,
                {ACCESS_CONTROL_ALLOW_ORIGIN, {"allow", "GET, HEAD, OPTIONS, PATCH, POST, PUT"}, ACCEPT_PATCH}, ""});
