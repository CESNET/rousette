...
TEST_CASE("HTTP")
{
  ...
  <location>location</location>
    ...
    SECTION("PUT")
    {
        ...
        REQUIRE(put("/ietf-system:system", R"({"ietf-system:system":{"ietf-system:location":"prague"}}")", {{"x-remote-user", "yangnobody"}, CONTENT_TYPE_JSON}) == Response{403, jsonHeaders, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "application",
  ...
  "example:a": {
    ...
    "example-augment:b": {
      "c": {
        ...
        REQUIRE(put("/example:l/list=netconf/choice1", R"({"example:choice1": "netconf"})", {AUTH_ROOT, CONTENT_TYPE_JSON}) == Response{204, jsonHeaders, ""});
