...
class ChangeMock {
public:
    MAKE_MOCK3(change, void(const sysrepo::ChangeOperation&, const std::string& path, const std::optional<std::string_view>& previousValue));
};
...
TEST_CASE("HTTP")
{
    ...
    auto sub2 = srSess.onModuleChange("example", [&](auto session, auto, auto, auto, auto, auto) {
        for (const auto& change: session.getChanges("/example:*//.")) {
            changeMock.change(change.operation, change.node.schema().path(), change.previousValue);
        }
        return sysrepo::ErrorCode::Ok;
    });
