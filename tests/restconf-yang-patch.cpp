...
TEST_CASE("YANG patch")
{
  ...
  "ietf-yang-patch:yang-patch-status": {
    ...
    REQUIRE(patch(RESTCONF_DATA_ROOT, {AUTH_ROOT, CONTENT_TYPE_YANG_PATCH_XML}, R"(
<yang-patch xmlns="urn:ietf:params:xml:ns:yang:ietf-yang-patch">
  <patch-id>patch</patch-id>
  <edit>
    <edit-id>edit</edit-id>
    <operation>replace</operation>
    <target>/example:top-level-leaf</target>
    <value xmlns:example="http://example.tld/example">
      <example:top-level-leaf>sorry not sorry</example:top-level-leaf>
    </value>
  </edit>
</yang-patch>
)") == Response{200, xmlHeaders, R"(<yang-patch-status xmlns="urn:ietf:params:xml:ns:yang:ietf-yang-patch">
  <patch-id>patch</patch-id>
  <ok/>
</yang-patch-status>
)"});
  ...
  "ietf-yang-patch:yang-patch-status": {
    ...
    // missing patch-id
    REQUIRE(patch(RESTCONF_DATA_ROOT "/example:tlc", {AUTH_ROOT, CONTENT_TYPE_YANG_PATCH_JSON}, R"({"ietf-yang-patch:yang-patch" : {}})") == Response{400, jsonHeaders, R"({
  "ietf-restconf:errors": {
    "error": [
      {
        "error-type": "protocol",
        "error-tag": "invalid-value",
        "error-message": "Expected data node 'patch-id' not found."
      }
    ]
  }
}
  ...
  "ietf-yang-patch:yang-patch-status": {
    ...
    // missing namespace in the value node
    REQUIRE(patch(RESTCONF_DATA_ROOT, {AUTH_ROOT, CONTENT_TYPE_YANG_PATCH_XML}, R"(
<yang-patch xmlns="urn:ietf:params:xml:ns:yang:ietf-yang-patch">
  <patch-id>patch</patch-id>
  <edit>
...
)") == Response{400, xmlHeaders, R"(<yang-patch-status xmlns="urn:ietf:params:xml:ns:yang:ietf-yang-patch">
  <patch-id>patch</patch-id>
  <edit-status>
    <edit>
      <edit-id>edit</edit-id>
      <errors>
        <error>
          ...
          <error-message>Validation failure: Can't parse data: LY_EVALID</error-message>
        </error>
      </errors>
    </edit>
  </edit-status>
  ...
  "example:ordered-lists": {
    ...
    // missing point with where=after
    REQUIRE(patch(RESTCONF_DATA_ROOT "/example:ordered-lists", {AUTH_ROOT, CONTENT_TYPE_YANG_PATCH_JSON}, R"({
  "ietf-yang-patch:yang-patch" : {
    "patch-id" : "patch",
    "edit" : [
  ...
  "ietf-yang-patch:yang-patch-status": {
            ...
            "error": [
              {
                "error-type": "protocol",
                "error-tag": "invalid-value",
                "error-message": "Required leaf 'point' not set."
              }
            ]
          }
        }
    ...
    // point with where not set to after/before
    REQUIRE(patch(RESTCONF_DATA_ROOT "/example:ordered-lists", {AUTH_ROOT, CONTENT_TYPE_YANG_PATCH_JSON}, R"({
  "ietf-yang-patch:yang-patch" : {
    "patch-id" : "patch",
    "edit" : [
            ...
            "error": [
              {
                "error-type": "protocol",
                "error-tag": "invalid-value",
                "error-message": "Leaf 'point' must always come with leaf 'where' set to 'before' or 'after'"
              }
            ]
          }
        }
