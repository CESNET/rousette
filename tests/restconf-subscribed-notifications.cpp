...
std::string establishSubscription(const libyang::Context& ctx, const libyang::DataFormat rpcEncoding, const auto& rpcRequestAuthHeader, const std::optional<std::string>& encodingLeafValue)
{
...
TEST_CASE("RESTCONF subscribed notifications")
{
    ...
    SECTION("XML stream")
    {
        ...
        SECTION("Inferred from XML content-type")
        {
            rpcRequestEncoding = libyang::DataFormat::XML;
        }
        SECTION("Explicitly asked for XML steam")
        {
            rpcSubscriptionEncoding = "encode-xml";
            SECTION("Content-type JSON")
            {
                rpcRequestEncoding = libyang::DataFormat::JSON;
            }
            SECTION("Content-type XML")
            {
                rpcRequestEncoding = libyang::DataFormat::XML;
            }
        }
    ...
    SECTION("JSON stream")
    {
        ...
        SECTION("Content-type with set encode leaf")
        {
            ...
            SECTION("Stream encoding inferred from RPC content-type")
            {
                rpcRequestEncoding = libyang::DataFormat::JSON;
            }
        }
