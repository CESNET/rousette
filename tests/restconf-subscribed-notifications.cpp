    ...
    const SubscribedNotifications& params)
{
    ...
    if (std::holds_alternative<std::string>(params.filter)) {
        rpcTree.newPath("stream-xpath-filter", std::get<std::string>(params.filter));
    } else if (std::holds_alternative<libyang::DataNode>(params.filter)) {
        rpcTree.newPath2("stream-subtree-filter", std::get<libyang::DataNode>(params.filter));
    }
    ...
    SECTION("Subscribed notifications requests")
    {
        ...
        SECTION("JSON stream")
        {
            ...
            SECTION("Subtree filter set")
            {
                ...
                subNotif.filter = *srSess.getContext().parseOp("<eventA xmlns='http://example.tld/example' />"s, libyang::DataFormat::XML, libyang::OperationType::NotificationYang).tree;
                rpcSubscriptionEncoding = "encode-json";
                EXPECT_NOTIFICATION(notificationsJSON[0], seq1);
                EXPECT_NOTIFICATION(notificationsJSON[3], seq1);
            }

            SECTION("Subtree filter with two notifications")
            {
                rpcRequestAuthHeader = AUTH_ROOT;
                rpcRequestEncoding = libyang::DataFormat::JSON;
                subNotif.filter = *srSess.getContext().parseOp("<eventA xmlns='http://example.tld/example' />"s, libyang::DataFormat::XML, libyang::OperationType::NotificationYang).tree;
                auto eventB = *srSess.getContext().parseOp("<eventB xmlns='http://example.tld/example' />"s, libyang::DataFormat::XML, libyang::OperationType::NotificationYang).tree;
                std::get<libyang::DataNode>(subNotif.filter).merge(eventB);
                rpcSubscriptionEncoding = "encode-json";
                EXPECT_NOTIFICATION(notificationsJSON[0], seq1);
                EXPECT_NOTIFICATION(notificationsJSON[1], seq1);
                EXPECT_NOTIFICATION(notificationsJSON[3], seq1);
