    ...
    const std::optional<sysrepo::NotificationTimeStamp>& replayStartTime)
{
    ...
    if (std::holds_alternative<libyang::DataNode>(filter)) {
        // FIXME: this needs libyang-cpp's support for lyd_new_path2 with LYD_NEW_PATH_ANY_DATATREE to avoid double parsing
        rpcTree.newPath2("stream-subtree-filter", *std::get<libyang::DataNode>(filter).printStr(libyang::DataFormat::JSON, libyang::PrintFlags::Shrink | libyang::PrintFlags::Siblings));
    }
