...
TEST_CASE("Terminating server under notification load")
{
    ...
    auto notificationThread = std::jthread(wrap_exceptions_and_asio(bg, io, [&]() {
        ...
        while (serverRunning) {
            ...
            notificationCount += 1;
            std::this_thread::sleep_for(2ms); // FIXME: Weirdly, without a short under tsan/asan, the notifications are being processed but client does not receive them.
        }
    }));

    auto serverShutdownThread = std::jthread([&]() {
        while (notificationCount <= NOTIFICATIONS_BEFORE_TERMINATE) {
    ...
    netconfWatcher.setDataFormat(libyang::DataFormat::JSON);
    REQUIRE_CALL(netconfWatcher, data(notif)).IN_SEQUENCE(seq1).TIMES(AT_LEAST(NOTIFICATIONS_BEFORE_TERMINATE));
