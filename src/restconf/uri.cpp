...
const auto nmdaDatastore = x3::rule<class nmdaDatastore, URIPrefix>{"uriPrefix"} = x3::attr(URIPrefix::Type::NMDADatastore) >> "/" >> fullyQualifiedApiIdentifier;
...
RestconfRequest asRestconfRequest(const libyang::Context& ctx, const std::string& httpMethod, const std::string& uriPath, const std::string& uriQueryString)
{
    ...
    if (httpMethod == "OPTIONS") {
        ...
        if (uri->prefix.resourceType == impl::URIPrefix::Type::RestconfRoot) {
            type = RestconfRequest::Type::RestconfRoot;
        } else if (uri->prefix.resourceType == impl::URIPrefix::Type::BasicRestconfOperations) {
