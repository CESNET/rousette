...
bool isValidDataResource(libyang::SchemaNode node) {
    switch(node.nodeType()) {
        ...
        case libyang::NodeType::Leaflist: // querying the actual (leaf-)list node is not a valid data resource. Only (leaf-list) entries are
            return true;
        case libyang::NodeType::List:
            return node.asList().keys().size() > 0; // key-less list is never a valid data resource
        default:
            return false;
    }
}
