...
const auto sseProxyStream = x3::rule<class sseProxyStream, SseProxyRequest>{"SSE proxy stream"} = x3::lit("rousette:sse-proxy") > x3::lit("/") > identifier;
...
const auto streamGrammar = x3::rule<class streamGrammar, std::variant<NotificationStreamRequest, SubscribedStreamRequest, SseProxyRequest>>{"stream URI"} =
    x3::lit("/") > x3::lit("streams") > x3::lit("/") > (subscribedStream | sseProxyStream | notificationStream);
...
std::variant<NotificationStreamRequest, SubscribedStreamRequest, SseProxyRequest> parseStreamUri(const std::string& input)
{
    return parse<std::variant<NotificationStreamRequest, SubscribedStreamRequest, SseProxyRequest>, UriPathSyntaxError>(input, streamGrammar);
}
...
SseProxyRequest::SseProxyRequest() = default;

SseProxyRequest::SseProxyRequest(const std::string& name)
    : name(name)
{
}
