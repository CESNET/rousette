...
const auto uriPath = x3::rule<class uriPath, std::vector<PathSegment>>{"uriPath"} = -("/" >> fullyQualifiedListInstance >> -("/" >> listInstance % "/")); // RFC 8040, sec 3.5.3
...
namespace {
sysrepo::Datastore datastoreFromApiIdentifier(const boost::optional<ApiIdentifier>& datastore)
{
    if (!datastore) {
