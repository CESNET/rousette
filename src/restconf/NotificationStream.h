...
public:
    ...
    DynamicSubscriptions(sysrepo::Connection& conn);
...
private:
    std::mutex m_mutex;
    std::map<boost::uuids::uuid, std::shared_ptr<SubscriptionData>> m_subscriptions; //! subscriptions indexed by a UUID of the subscription
