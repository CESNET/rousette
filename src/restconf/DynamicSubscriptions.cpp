    ...
    const std::string& filterNamePath)
{
    ...
    if (auto node = rpcInput.findPath(filterNamePath)) {
        ...
        spdlog::error("QUERY = {}", xpath);
        spdlog::error("DATA = {}", *session.getData("/ietf-subscribed-notifications:filters")->printStr(libyang::DataFormat::XML, libyang::PrintFlags::Siblings));
