...
SseProxy::SourcesByEndpoint collectSources(sysrepo::Connection& conn, const libyang::DataNode& data)
{
    ...
    // create endpoints
    SseProxy::SourcesByEndpoint byEndpoint;
...
void SseProxy::start()
{
    ...
    if (!sseProxyEnabled(session.getContext())) {
        spdlog::debug("The {} feature is not enabled, not serving any SSE proxy endpoints", sseProxyFeature);
        return;
    }
...
void SseProxy::reconfigure(sysrepo::Session session)
{
...

    {
        ...
        for (auto it = m_endpoints.begin(); it != m_endpoints.end();) {
            ...
            spdlog::info("SSE proxy '{}' is gone from the configuration", it->first);
        ...
        for (auto& [name, feeds] : byEndpoint) {
            ...
            if (auto it = m_endpoints.find(name); it != m_endpoints.end()) {
                ...
                spdlog::debug("SSE proxy '{}' now fed by {} subscription(s)", name, count);
            } else {
