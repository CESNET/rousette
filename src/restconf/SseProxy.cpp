...
void SseProxy::start()
{
    ...
    m_confUpdates = session.onModuleChange(
...
void SseProxy::reconfigure(sysrepo::Session session)
{
    ...
    std::vector<std::unique_ptr<SseProxyEndpoint>> toDelete; // endpoints to be destroyed on the IO thr

    {
        ...
        for (auto it = m_endpoints.begin(); it != m_endpoints.end();) {
            ...
            toDelete.push_back(std::move(m_endpoints.extract(it++).mapped()));
        }
    ...
    // defer deleting endpoints on the IO thread
    SseProxyEndpoint::deleteLater(io, std::move(toDelete));
}
