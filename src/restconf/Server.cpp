    ...
    , dwdmEvents{std::make_unique<sr::OpticalEvents>(conn.sessionStart())}
{
    ...
    server->handle(restconfRoot,
        [conn /* intentionally by value, otherwise conn gets destroyed when the ctor returns */, this](const auto& req, const auto& res) mutable {
            ...
            if (!path) {
                rejectWithError(sess, req, res, 400, "application", "operation-failed", "Not a subtree path."); // FIXME: Is this correct error? This is what Netopeer2 returns when invalid path is supplied.
                return;
            }

            try {
                if (!hasModuleForPath(sess, *path)) {
                    rejectWithError(sess, req, res, 400, "application", "operation-failed", "Module not found.");
                    return;
                }
