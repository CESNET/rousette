...
void processPut(std::shared_ptr<RequestContext> requestCtx)
{
    ...
    try {
        ...
        if (!lyPathParent.empty()) {
            ...
            if (children.size() != 1) {
                rejectWithError(requestCtx->sess.getContext(), requestCtx->dataFormat.response, requestCtx->req, requestCtx->res, 400, "application", "operation-failed", "Invalid data for PUT (multiple or none top-level elements).");
                return;
            }
        ...
        if (replacementNode->schema().name() != lastPathSegment.apiIdent.identifier || std::string(replacementNode->schema().module().name()) != lastPathSegment.apiIdent.prefix) {
            rejectWithError(requestCtx->sess.getContext(), requestCtx->dataFormat.response, requestCtx->req, requestCtx->res, 400, "application", "operation-failed", "Invalid data for PUT (Top-level node name mismatch).");
            return;
        }
