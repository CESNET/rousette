...
bool sameNode(const libyang::DataNode& child, const PathSegment& lastPathSegment)
{
    return child.schema().module().name() == *lastPathSegment.apiIdent.prefix && child.schema().name() == lastPathSegment.apiIdent.identifier;
}

bool sameNode(const libyang::DataNode& a, const libyang::Leaf& b)
{
        ...
        if (!lyPathParent.empty()) {
            ...
            auto isKeyNode = [](const auto& maybeList, const auto& node) {
                if (maybeList->schema().nodeType() == libyang::NodeType::List) {
                    ...
                    return std::find_if(listKeys.begin(), listKeys.end(), [&node](const auto& key) { return sameNode(node, key); }) != listKeys.end();
                }
                return false;
            };
            for (const auto& child : node->immediateChildren()) {
                 ...
                 *  - a single child that is created by parseSubtree with the name of lastPathSegment
                 * anything else is an error (either too much children provided or invalid name)
                 */
                if (isKeyNode(node, child)){
                    continue;
                } else if (sameNode(child, lastPathSegment)) {
                    replacementNode = child;
                } else {
                    rejectWithError(requestCtx->sess.getContext(), requestCtx->dataFormat.response, requestCtx->req, requestCtx->res, 400, "application", "operation-failed", "Invalid data for PUT (data contains invalid node).", child.path());
                    return;
                }
            }
            ...
            if (!sameNode(*replacementNode, lastPathSegment)) {
                rejectWithError(requestCtx->sess.getContext(), requestCtx->dataFormat.response, requestCtx->req, requestCtx->res, 400, "application", "operation-failed", "Invalid data for PUT (data contains invalid node).", replacementNode->path());
                return;
            }
        }
            ...
            try {
                ...
                if (req.method() == "GET") {
                    ...
                    if (!dataFormat.request) {
                        rejectWithError(sess.getContext(), dataFormat.response, req, res, 400, "protocol", "invalid-value", "Content-type header missing.");
                        return;
                    }
