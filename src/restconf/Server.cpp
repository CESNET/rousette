        ...
        if (!lyPathParent.empty()) {
            ...
            auto isKeyNode = [](const auto& maybeList, const auto& node) {
                if (maybeList->schema().nodeType() == libyang::NodeType::List) {
                    ...
                    return std::find_if(listKeys.begin(), listKeys.end(), [&node](const auto& key) { return key.name() == node.schema().name(); }) != listKeys.end();
                }
                return false;
            };
            for (const auto& child : node->immediateChildren()) {
                ...
                if (isKeyNode(node, child)){
                    continue;
                } else if (child.schema().module().name() == *lastPathSegment.apiIdent.prefix && child.schema().name() == lastPathSegment.apiIdent.identifier) {
                    replacementNode = child;
                } else {
                    rejectWithError(requestCtx->sess.getContext(), requestCtx->dataFormat.response, requestCtx->req, requestCtx->res, 400, "application", "operation-failed", "Invalid data for PUT (data contains invalid node).");
                    return;
                }
            }
