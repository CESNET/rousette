...
/** @brief Iterates through parsed tree nodes. Rejects the input if any meta attributes that could possibly alter sysrepo's behaviour are found. */
void validateInputMetaAttributes(const libyang::Context& ctx, const libyang::DataNode& tree)
{
    ...
    for (const auto& node : tree.childrenDfs()) {
        ...
        for (const auto& metaEntry : node.meta()) {
            // internal libyang meta that sometimes silently appears
            if (metaEntry.module() == modYang && metaEntry.name() == "lyds_tree") {
                continue;
            }

            if (metaEntry.module() == modNetconf || metaEntry.module() == modYang || metaEntry.module() == modSysrepo) {
                throw ErrorResponse(400, "application", "invalid-value", "Meta attribute from '" + metaEntry.module().name() + "' module found in data tree");
            }
        }
    }
}
