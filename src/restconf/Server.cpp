...
void processPut(std::shared_ptr<RequestContext> requestCtx)
{
    ...
    try {
        ...
        auto isKeyNode = [](const auto& maybeList, const auto& node) {
        ...
        auto getKeyPosition = [&](const libyang::DataNode& lst, const std::string_view& keyName) {
        ...
        if (!lyPathParent.empty()) {
            ...
            for (const auto& child : node->immediateChildren()) {
                 ...
                 */
                if (isKeyNode(*node, child)) {
                    continue;
                } else if (isSameNode(child, lastPathSegment)) {
                    if (child.schema().nodeType() == libyang::NodeType::List) { // check keys if we inserted into a list
                        for (const auto& listChild : child.immediateChildren()) {
                            if (isKeyNode(child, listChild)) {
                                if (auto keyValue = lastPathSegment.keys[getKeyPosition(child, listChild.schema().name())]; keyValue != listChild.asTerm().valueStr()) {
                                    throw ErrorResponse(400, "application", "operation-failed", "Invalid data for PUT (list key mismatch between URI path and data).", listChild.path());
                                }
                            }
                        }
                    } else if (child.schema().nodeType() == libyang::NodeType::Leaflist) {
                        if (auto keyValue = lastPathSegment.keys[0]; keyValue != child.asTerm().valueStr()) {
                            throw ErrorResponse(400, "application", "operation-failed", "Invalid data for PUT (list key mismatch between URI path and data).", child.path());
                        }
                    }
            ...
            if (parent->schema().nodeType() == libyang::NodeType::List) { // check keys if we inserted into a list
                for (const auto& listChild : parent->immediateChildren()) {
                    if (isKeyNode(*parent, listChild)) {
                        if (auto keyValue = lastPathSegment.keys[getKeyPosition(*parent, listChild.schema().name())]; keyValue != listChild.asTerm().valueStr()) {
                            throw ErrorResponse(400, "application", "operation-failed", "Invalid data for PUT (list key mismatch between URI path and data).", listChild.path());
                        }
                    }
                }
            }
            if (parent->schema().nodeType() == libyang::NodeType::Leaflist) {
                if (auto keyValue = lastPathSegment.keys[0]; keyValue != parent->asTerm().valueStr()) {
                    throw ErrorResponse(400, "application", "operation-failed", "Invalid data for PUT (list key mismatch between URI path and data).", parent->path());
                }
            }
        }
