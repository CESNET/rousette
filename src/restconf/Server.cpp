...
{
    return notifPrefix + libyang::yangTimeFormat(time, libyang::TimezoneInterpretation::Unspecified) + notifMid + content + notifSuffix;
}
    ...
    , dwdmEvents{std::make_unique<sr::OpticalEvents>(conn.sessionStart())}
{
    ...
    server->handle(netconfStreamRoot, [this, conn](const auto& req, const auto& res) mutable {
        ...
        try {
            ...
            // RFC does not specify how the errors should look like so let's just report the HTTP code
            res.write_head(e.code, {{"access-control-allow-origin", {"*", false}}});
            res.end();
            return;
        }
