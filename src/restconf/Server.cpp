    ...
    server->handle(netconfStreamRoot, [this, conn](const auto& req, const auto& res) mutable {
        ...
        try {
            ...
            if (std::holds_alternative<RestconfStreamRequest::SubscribedNotification>(streamRequest.type)) {
                auto ypSub = m_dynamicSubscriptions.getSubscription(std::get<RestconfStreamRequest::SubscribedNotification>(streamRequest.type).uuid);
                if (!ypSub) {
                    throw ErrorResponse(404, "application", "no-such-subscription", "Subscription not found.");
                }

                auto client = std::make_shared<DynamicSubscriptionHttpStream>(req, res, std::make_shared<rousette::http::EventStream::Signal>(), sess, libyang::DataFormat::JSON, ypSub);
                client->activate();
            } else {
