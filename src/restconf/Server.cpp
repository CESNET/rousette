...
void yangInsert(const libyang::Context& ctx, libyang::DataNode& listEntryNode, const std::string& where, const std::optional<queryParams::insert::PointParsed>& pointParsed)
{
    ...
    if (pointParsed) {
        ...
        std::string keyOrValueAttribute;
        ...
        if (listEntrySchema.nodeType() == libyang::NodeType::List) {
            keyOrValueAttribute = listKeyPredicate(listEntrySchema.asList().keys(), pointParsed->back().keys);
        } else if (listEntrySchema.nodeType() == libyang::NodeType::Leaflist) {
            keyOrValueAttribute = pointParsed->back().keys[0];
        } else {
            throw std::logic_error("Node is neither a list nor a leaf-list");
        }

        listEntryNode.newMeta(modYang, listEntryNode.schema().nodeType() == libyang::NodeType::List ? "key" : "value", keyOrValueAttribute);
    }
}
 ...
 */
libyang::CreatedNodes createEditForPutAndPatch(libyang::Context& ctx, const std::string& uriPath, const std::optional<std::string>& valueStr, const std::optional<libyang::DataFormat>& dataFormat)
{
    ...
    if (!lyParentPath.empty() && valueStr) {
        ...
        editNode = parent;
    } else if (lyParentPath.empty() && valueStr) {
...
void processYangPatchImpl(const std::shared_ptr<RequestContext>& requestCtx, const std::optional<libyang::DataNode>& patch, const std::string& patchId)
{
    auto ctx = requestCtx->sess.getContext();
    ...
    for (const auto& editContainer : patch->findXPath("edit")) {
        ...
        WITH_RESTCONF_EXCEPTIONS(processYangPatchEdit, [&](libyang::Context ctx, const libyang::DataFormat& dataFormat, const request& req, const response& res, const int code, const std::string errorType, const std::string& errorTag, const std::string& errorMessage, const std::optional<std::string>& errorPath) {
        ...
        (requestCtx, editContainer, mergedEdits);
    }
...
void processYangPatch(std::shared_ptr<RequestContext> requestCtx)
{
    ...
    WITH_RESTCONF_EXCEPTIONS(processYangPatchImpl, [&](libyang::Context ctx, const libyang::DataFormat& dataFormat, const request& req, const response& res, const int code, const std::string errorType, const std::string& errorTag, const std::string& errorMessage, const std::optional<std::string>& errorPath) {
        auto ext = ctx.getModuleImplemented("ietf-yang-patch")->extensionInstance("yang-patch-status");
...
{
    constexpr auto operationsXPath = "/ietf-restconf:restconf/operations";
    auto yangApiExt = ctx.getModuleImplemented("ietf-restconf")->extensionInstance("yang-api");

    auto parent = ctx.newExtPath(operationsXPath, std::nullopt, yangApiExt);
    auto operations = *parent->findXPath(operationsXPath).begin();
    ...
    for (const auto& mod : ctx.modules()) {
        ...
        for (const auto& rpc : mod.rpcList()) {
            // DOES NOT WORK
            // operations.newPath(rpc.module().name() + ":" + rpc.name(), std::nullopt, libyang::CreationOptions::Opaque);
        ...
        [conn /* intentionally by value, otherwise conn gets destroyed when the ctor returns */, this](const auto& req, const auto& res) mutable {
            ...
            try {
                ...
                case RestconfRequest::Type::ListRPC: {
                    ...
                    res.end(*listRPCs(sess.getContext()).printStr(dataFormat.response, libyang::PrintFlags::WithSiblings));
                    break;
                }
                }
            } catch (const auth::Error& e) {
