    ...
    , dwdmEvents{std::make_unique<sr::OpticalEvents>(conn.sessionStart())}
{
    ...
    server->handle(netconfStreamRoot, [this, conn](const auto& req, const auto& res) mutable {
        ...
        try {
            ...
            if (streamRequest.type == RestconfStreamRequest::Type::SubscribedNotification) {
                auto client = std::make_shared<SubscribedNotificationStream>(req, res, std::make_shared<rousette::http::EventStream::Signal>(), sess, libyang::DataFormat::JSON, m_subscribedNotifications.getSubscription(1));
                client->activate();
            } else {
