...
using UserPass = std::pair<std::string, std::string>;
...
UserPass parseBasicAuth(const std::string& blob)
{
    ...
    if (!x3::parse(iter, end, headerGrammar >> x3::eoi, b64)) {
        throw restconf::ErrorResponse(401, "protocol", "access-denied", "Cannot parse the Basic authorization header");
    }
    ...
    if (!x3::parse(iter, end, userPass >> x3::eoi, res)) {
        throw restconf::ErrorResponse(401, "protocol", "access-denied", "Cannot parse the user-pass authorization blob");
    }
    return res;
}
...
static int pam_userpass_conv(int num_msg, const struct pam_message **msg, struct pam_response **resp_r, void *appdata_ptr)
{
    ...
    auto release_response = [&num_msg](struct pam_response *resp) {
        spdlog::info("pam_userpass_conv: release_response");
        for (int i = 0; i < num_msg; ++i) {
            if (resp[i].resp) {
                spdlog::info("pam_userpass_conv: free(resp[i].resp)");
                free(resp[i].resp);
            }
        }
        spdlog::info("pam_userpass_conv: free(resp)");
        free(resp);
    };
    ...
    for (int i = 0; i < num_msg; ++i) {
        ...
        switch (msg[i]->msg_style) {
            case PAM_PROMPT_ECHO_ON:
                ...
                str = strdup(userPass.first.c_str());
                if (!str) {
                    throw std::runtime_error{"PAM: strdup(username) failed"};
                }
                spdlog::info("pam_userpass_conv: setting username {}", str);
                break;
            case PAM_PROMPT_ECHO_OFF:
                // assume that this is the password
                str = strdup(userPass.first.c_str());
                if (!str) {
                    throw std::runtime_error{"PAM: strdup(password) failed"};
                }
                spdlog::info("pam_userpass_conv: setting password {}", str);
                break;
            /* case PAM_ERROR_MSG: */
            /* case PAM_TEXT_INFO: */
            default:
                throw std::logic_error{"PAM: pam_userpass_conv: unexpected msg_style " + std::to_string(msg[i]->msg_style)};
        }
...
std::string authenticate_pam(const UserPass& userPass, const std::optional<std::filesystem::path>& dir, const std::optional<std::string>& remoteHost)
{
    ...
    auto check = [&res, &pamh](const std::string& fun) {
        if (res != PAM_SUCCESS) {
            throw restconf::ErrorResponse{401, "protocol", "access-denied", "PAM access denied: " + fun + ": " + pam_strerror(pamh, res)};
        }
    };
    ...
    res = pam_start_confdir("rousette", userPass.first.c_str(), &conv, dir ? std::string(*dir).c_str() : nullptr, &pamh);
