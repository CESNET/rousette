...
{
    // first activate the signal and only then subscribe to sysrepo (so we do not lose any notifications)
    EventStream::activateSignal(m_notificationSignal);
    ...
    for (const auto& mod : m_session.getContext().modules()) {
        if (mod.implemented()) {
            try {
                 ...
                 */
                if (e.code() != sysrepo::ErrorCode::NotFound && (mod.name() != "sysrepo" || e.code() != sysrepo::ErrorCode::Internal)) { // FIXME: https://github.com/sysrepo/sysrepo/issues/3348
                    throw;
                }
            }
        }
