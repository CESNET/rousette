...
void DynamicSubscriptions::establishSubscription(const libyang::DataNode& rpcInput, libyang::DataNode& rpcOutput, const libyang::DataFormat requestEncoding)
{
    std::string stream;
    ...
    // TODO: We are not following the state machine from RFC8639 2.4.1, we are always in state "receiver active"
    ...
    auto sub = m_session.subscribeNotifications(
...
std::shared_ptr<SubscriptionData> DynamicSubscriptions::getSubscription(const boost::uuids::uuid uuid)
{
    std::unique_lock lock(m_mutex);
