...
{
    auto session = conn.sessionStart(sysrepo::Datastore::Operational);

    const auto replayInfo = sysrepoReplayInfo(session);
    static const auto prefix = "/ietf-subscribed-notifications:streams"s;
    session.setItem(prefix + "/stream[name='NETCONF']/description", "Default NETCONF notification stream");
    if (replayInfo.enabled) {
        session.setItem(prefix + "/stream[name='NETCONF']/replay-support", std::nullopt);
        if (replayInfo.earliestNotification) {
            session.setItem(prefix + "/replay-log-creation-time", libyang::yangTimeFormat(*replayInfo.earliestNotification, libyang::TimezoneInterpretation::Local));
        }
    }
}
