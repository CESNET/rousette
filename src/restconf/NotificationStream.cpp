...
const auto dummyPrefix = "http://localhost"s;
    ...
    : EventStream(req, res)
{
    ...
    for (const auto& mod : session.getContext().modules()) {
        if (mod.implemented()) {
            try {
                createNotificationSub(m_notifSubs, session, mod.name(), m_notificationSignal, dataFormat, filter, startTime, stopTime);
                ...
                if (e.code() != sysrepo::ErrorCode::NotFound &&
                    (mod.name() != "sysrepo" || e.code() != sysrepo::ErrorCode::Internal)) { // FIXME: https://github.com/sysrepo/sysrepo/issues/3348
                    throw;
                }
            }
        }
...
void notificationStreamList(sysrepo::Session& session, std::optional<libyang::DataNode>& parent, const std::string& streamsPrefix)
{
    ...
    for (const auto& mod : session.getContext().modules()) {
        if (mod.implemented()) {
            try {
                ...
                // In case the module is not found this exception is called
                // This happens only for the 'sysrepo' module
                if (e.code() != sysrepo::ErrorCode::NotFound) {
                    throw;
                }
            }
    ...
    parent->newPath(prefix + "/access[encoding='xml']/location", dummyPrefix + streamsPrefix + "NETCONF/XML");
    parent->newPath(prefix + "/access[encoding='json']/location", dummyPrefix + streamsPrefix + "NETCONF/JSON");
...
{
    // if we were unable to parse scheme and hosts, end without providing any streams
    // the YANG schema specifies that there must be at least one endpoint to each stream but we are unable to provide any
    if (!schemeAndHost) {
        std::vector<libyang::DataNode> nodesToDelete;
        auto nodes = node.findXPath(streamListXPath);
    ...
    for (const auto& n : accessNodes) {
        ...
        val = val.substr(dummyPrefix.size());
