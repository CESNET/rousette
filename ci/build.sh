...
pushd ~/$(jq < ~/zuul-env.json -r '.projects[] | select(.name == "github/nghttp2/nghttp2").src_dir')
autoreconf -i
automake
autoconf
./configure --prefix=${PREFIX} --enable-asio-lib --enable-app --disable-dependency-tracking --disable-python-bindings --disable-examples --disable-hpack-tools --enable-static=no
